...
class SSO_Bridge {
    ...
    public function add_rewrite_endpoint() {
        add_rewrite_endpoint( 'ce-login', EP_ROOT );
    }

    public function handle_sso_endpoint() {
        global $wp_query;
        if ( ! isset( $wp_query->query_vars['ce-login'] ) ) { return; }
        ...
        if ( empty( $sso_url ) ) { wp_die( esc_html__( 'SSO is not completely configured.', 'spiritwp-connect' ) ); }
